...
void convert_label_to_0_and_add_features(const string& input_csv, const string& output_csv) {
    ...
    while (getline(input, line)) {
        ...
        tokens.push_back((tokens[1] != "ssh" && tokens[1] != "other" && tokens[1] != "dns") ? "1" : "0"); // Other_Services

        tokens.push_back((tokens[24] == "tcp") ? "1" : "0"); // TCP_Protocol
        tokens.push_back((tokens[24] == "udp") ? "1" : "0"); // UDP_Protocol
        tokens.push_back((tokens[24] != "tcp" && tokens[24] != "udp") ? "1" : "0"); // Multiple_Other_Protocol
...
int main() {
    ...
    vector<int> data_counts = {150, 300, 450, 600, 750, 900, 1050, 1200, 1350, 1500};

    for (const string& data_csv : data_csvs) {
        for (const int& count : data_counts) {
            string input_csv = "./data/buf/" + data_csv + "_" + to_string(count) + ".csv";
            string output_csv = "./data/buf/" + data_csv + "_" + to_string(count) + "_value.csv";

            convert_label_to_0_and_add_features(input_csv, output_csv);

            cout << "Converted -1 labels to 0 and added new features in " << input_csv << " and saved to " << output_csv << endl;
        }
    }
